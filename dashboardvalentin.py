...
if uploaded_file is not None:
    ...
    if all(col in df.columns for col in colunas_esperadas):
        ...
        anos_por_cliente = df.groupby("Cliente")["Ano"].apply(list).reset_index()
        anos_por_cliente["Recorrência"] = anos_por_cliente["Ano"].apply(lambda x: len(set(x)))
        anos_por_cliente["Anos Ativos"] = anos_por_cliente["Ano"].apply(lambda x: ", ".join(map(str, sorted(set(x)))))
        ...
        base_filtrada = base_filtrada[
            base_filtrada["Anos Ativos"].apply(lambda x: any(str(ano) in x for ano in anos_selecionados))
        ]
        ...
        base_exibicao = base_filtrada[["Cliente", "Total Gasto", "Recorrência", "Média Anual", "Anos Ativos", "Intervalo Sem Compra"]]
